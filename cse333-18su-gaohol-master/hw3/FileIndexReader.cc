...
FileIndexReader::FileIndexReader(std::string filename,
  ...
  // Done:
  HWSize_t res = fread(&header_, sizeof(header_), 1, file_);
  Verify333(res == 1);
  ...
  if (validate) {
    ...
    while (left_to_read > 0) {
      // Done:
      // read 512 bytes at one time
      HWSize_t rlen = fread(buf, sizeof(char), 512, file_);
      Verify333(rlen > 0);
      for (HWSize_t i = 0; i < rlen; i++) {
        // get crc
        crcobj.FoldByteIntoCRC(buf[i]);
      }
      left_to_read -= rlen;
    }
    // check if currect calculated checksum is not corrupted
