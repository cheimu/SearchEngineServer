...
HashTableReader::LookupElementPositions(HTKey_t hashKey) {
  ...
  HWSize_t res = fseek(file_, bucketrec_offset, SEEK_SET);
  Verify333(res == 0);
  // read chain len and bucket position
  ...
  Verify333(res == 0);
  int chainLen = b_rec.chain_len;
  for (int i = 0; i < chainLen; i++) {
    element_position_rec elementPosRec;
    // read the element positions
    res = fread(&elementPosRec, sizeof(elementPosRec), 1, file_);
    Verify333(res == 1);
    // get it to host order
    elementPosRec.toHostFormat();
    // insert into the list
    retval.push_back(elementPosRec.element_position);
  }
  // Return the list.
  return retval;
}
