...
static HWSize_t WriteDocidDocnameFn(FILE *f,
  ...
  // Done: (change this assignment to the correct thing):
  ...
  // DONE (change this return to the correct thing):
  return tlen;
}
...
static HWSize_t WriteDocPositionListFn(FILE *f,
  ...
  for (i = 0; i < num_pos_ho; i++) {
    ...
    // DONE:
    position = {*reinterpret_cast<DocPositionOffset_t*>(&payload)};
    position.toDiskFormat();
    // find the correct position
    if (fseek(f, offset+tlen, SEEK_SET) != 0) {
      LLIteratorFree(it);
      return 0;
    }
    // write it into file
...
static HWSize_t WriteHeader(FILE *f,
  ...
  for (HWSize_t i = 0; i < cslen; i++) {
    ...
    crcobj.FoldByteIntoCRC(static_cast<uint8_t>(buf));
  }
...
static HWSize_t WriteBucket(FILE *f,
  ...
  if (chainlen_ho > 0) {
    ...
    for (j = 0; j < chainlen_ho; j++) {
      ...
      // Done:
      element_position_rec header = {static_cast<IndexFileOffset_t>(nextelpos)};
      // set header into network order
      header.toDiskFormat();
      // get the correct location
      if (fseek(f, offset + j * sizeof(header), SEEK_SET) != 0) {
        LLIteratorFree(it);
        return 0;
      }
      // write in into file
...
static HWSize_t WriteHashTable(FILE *f,
  ...
  for (i = 0; i < ht->num_buckets; i++) {
    ...
    // check if bucket chain is empty
    if (NumElementsInLinkedList(li) == 0) {
      res = 0;
    } else {
      // if not empty, write a bucket
