...
static void LoopAndInsert(HashTable tab, char *content) {
  ...
  while (1) {
    ...
    if (*curptr == '\0') {
      return;
      // check if it is normal character
    } else if (isalpha(*curptr) != '\0') {
      *curptr = tolower(*curptr);
    } else {
      *curptr = '\0';
      // check if it is start of a word
...
static void AddToHashTable(HashTable tab, char *word, DocPositionOffset_t pos) {
  ...
  if (retval == 1) {
    ...
    // how we're casting the DocPositionOffset_t position variable to
    // an LLPayload_t to store it in the linked list payload without needing
    // to malloc space for it. Ugly, but it works!
    ...
    char *newstr;
    HTKeyValue old;
    // allocate space for new wordposition
    ...
    wp->word = newstr;
    // prepare linkedlist for wordposition
    wp->positions = AllocateLinkedList();
    Verify333(wp->positions != NULL);
    // append new wordposition to its linkedlist
    retval = AppendLinkedList(wp->positions, (LLPayload_t) ((intptr_t) pos));
    Verify333(retval != 0);
    kv.key = (HTKey_t) hashKey;
    kv.value = (HTValue_t) wp;
    // insert key and wordposition to hashtable
