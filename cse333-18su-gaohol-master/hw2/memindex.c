...
int MIAddPostingList(MemIndex index, char *word, DocID_t docid,
  ...
  if (res == 0) {
    ...
    wds->word = word;
    // allocate new hshtable for docID->positions
    wds->docIDs = AllocateHashTable(128);
    Verify333(wds->docIDs != NULL);
    // insert hashtable into wordDocSet
    kv.key = wordkey;
    kv.value = (HTValue_t)wds;
    // insert the new wordDocSet into the inverted index
    int opResult = InsertHashTable(index, kv, &hitkv);
    Verify333(opResult == 1);
  } else {
...
LinkedList MIProcessQuery(MemIndex index, char *query[], uint8_t qlen) {
  ...
  // look up the corresponding hashcode in hashtable
  res = LookupHashTable(index, wordkey, &kv);
  Verify333(res != -1);
  // if no ducoment matches
  if (res == 0) {
  ...
  // iterate through all the documents in hashtable
  for (int i = 0; i < size; i++) {
    int opResult = HTIteratorGet(htit, &kv);
    Verify333(opResult != 0);
    SearchResult* sr = (SearchResult *) malloc(sizeof(SearchResult));
    Verify333(sr != NULL);
    // get document id
    sr->docid = kv.key;
    // get rank
    sr->rank = (HWSize_t)NumElementsInLinkedList((LinkedList) kv.value);
  ...
  for (i = 1; i < qlen; i++) {
    ...
    for (j = 0; j < ne; j++) {
      ...
      // check if documents are in the set of matches for query[i]
      res = LookupHashTable(wds->docIDs, sr->docid, &kv);
      Verify333(res != -1);
      if (res == 1) {
        // if exsit, then update the new rank
        sr->rank += (HWSize_t)NumElementsInLinkedList((LinkedList) (kv.value));
        LLIteratorNext(llit);
      } else {
        // if not, delete the document id
