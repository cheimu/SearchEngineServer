...
DocID_t DTRegisterDocumentName(DocTable table, char *docname) {
  ...
  // STEP 2.
  // get the hashcode
  res = FNVHash64((unsigned char *) doccopy, (HWSize_t) strlen(doccopy));
  // look up the document and check if it already exists
  int opResult = LookupHashTable(table->docname_to_docid, res, &oldkv);
  Verify333(opResult != -1);
  // if exsits, free up all the space
  if (opResult == 1) {
    free(docid);
    free(doccopy);
  ...
  *docid = table->max_id;
  ...
  kv.key = *docid;
  kv.value = doccopy;
  opResult = InsertHashTable(table->docid_to_docname, kv, &oldkv);
  Verify333(opResult == 1);
  ...
  kv.key = res;
  kv.value = docid;
  opResult = InsertHashTable(table->docname_to_docid, kv, &oldkv);
  Verify333(opResult == 1);
  return *docid;
}
...
DocID_t DTLookupDocumentName(DocTable table, char *docname) {
  ...
  // docID if it is.
  // get hachcode
  key = FNVHash64((unsigned char *) docname, (HWSize_t) strlen(docname));
  // lookup the document name inside table of name to id
