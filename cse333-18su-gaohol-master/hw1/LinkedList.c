...
bool PopLinkedList(LinkedList list, LLPayload_t *payload_ptr) {
  ...
  *payload_ptr = temp->payload;
  // if current linkedlist has no node, then set both head and tail as null
...
bool LLIteratorDelete(LLIter iter,
  ...
  // degenerate case 0: the list becomes empty after deleting
  // return false if it succeeds to delete and becomes empty
  ...
  // degenerate case 1: iter points at head
  if (!LLIteratorHasPrev(iter)) {
    iter->node = iter->node->next;
    iter->list->head = iter->node;
    iter->node->prev = NULL;
  } else if (!LLIteratorHasNext(iter)) {
    // degenerate case 2: iter points at tail
    iter->node = iter->node->prev;
